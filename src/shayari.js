...
const getShayaris = async (rekhtaUrl, isSinglePoet, count) => {
	...
	const shayaris = await page.evaluate(
		(count, isSinglePoet) => {
			let shayariList = Array.from(document.querySelectorAll('.sherSection'));
...
const getShayarisByTag = async (tag, language, count, sort) => {
	...
	const shayaris = await getShayaris(url, false, count);
	return shayaris;
};
...
const getShayarisByPoet = async (poet, language, count, sort) => {
	...
	const shayaris = await getShayaris(url, true, count);
	return shayaris;
};
